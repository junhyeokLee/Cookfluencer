...
class RecentChannelScreen extends HookConsumerWidget {
  const RecentChannelScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final pagingController = useState(
      PagingController<String?, ChannelData>(firstPageKey: null),
    );

    // 🔄 Firestore 데이터 페이지별 로딩
    Future<void> fetchPage(String? lastUploadDate) async {
      try {
        final videos = await ref.read(paginatedRecentChannelsProvider(lastUploadDate).future);
    ...
    return Scaffold(
      ...
      body: PagedListView<String?, ChannelData>(
